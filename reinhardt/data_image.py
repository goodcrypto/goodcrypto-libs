...
def data_uri(data, mime_type, charset=None):
    ...
    # this is a bit of a kludge to handle differences between
    # python2 and python3 handling of strings, but it works
    base64_data = base64.b64encode(data)
    data_string = base64_data.decode('utf-8').replace('\n', '')
    if charset:
        uri = 'data:%s;%s;base64,%s' % (charset, mime_type, data_string)
    else:
        uri = 'data:%s;base64,%s' % (mime_type, data_string)
