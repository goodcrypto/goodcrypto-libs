...
import os, random, re, sh, sys, time, urlparse
...
class SearchEngine(object):
    ...
    def search(self, terms):
        ...
        search_query_url = urlparse.urljoin(self.form_url, action_url)
                ...
                if inputs is not None:
                    for input in inputs:
                        ...
                        if attrs['type'] == 'submit' and attrs['value'].startswith('Next'):
                            ...
                            action_url = form.attr('action')
                            print('action_url: %s' % action_url)
                            search_query_url = urlparse.urljoin(search_query_url, action_url)
                            data = self.get_form_data(form, terms)
                else:
