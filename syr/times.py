...
def timestamp(microseconds=True):
    ''' Return timestamp string in a standard format. Time zone is UTC.
...
def timedelta_to_human_readable(td, verbose=True):
    ...
    if td.days or td.seconds:
        ...
        # days
        if td.days:
            tdString = '{} day{}'.format(td.days, s_if_plural(td.days))
        ...
        if hours:
            if len(tdString) > 0:
                tdString = tdString + ', '
            tdString = tdString + '{} hour{}'.format(hours, s_if_plural(hours))
        ...
        if secondsLeft:
            ...
            if minutes:
                if len(tdString) > 0:
                    tdString = tdString + ', '
                tdString = tdString + '{} minute{}'.format(minutes, s_if_plural(minutes))
        ...
        if secondsLeft:
            if len(tdString) > 0:
                tdString = tdString + ', '
            tdString = tdString + '{} second{}'.format(secondsLeft, s_if_plural(secondsLeft))
...
def get_short_date_time(date_time):
    ...
    if date_time:
        new_date_time = date_time.isoformat(str(' '))
        try:
